...
__name__ = ['PyomoModelSolvingStatus','PyomoModel','PyomoSimulator','PyomoOptimizer',
           'PyomoModelWithModifiers','PyomoMultiDataPointParamEstimator',
...
class PyomoModelWithModifiers(PyomoModel):
    def __init__(self, pyomo_model, mvs, cvs):
        '''
        ...
        self.objective = pyomo_model.objective
    ...
    def build(self, model):
        self.base_pyomo_model.build(model)
        for name in self.modifier_names_iterator():
        ...
        for cv in self.cvs:
            temp_func=modified_output_function(self, cv, self.mvs)
...
class PyomoOptimizer(Optimizer):
    ...
    def build(self, problem_description, homotopy_factor=1000):
        ...
        # set obj
        def _objective(m):
            return self.pyomo_model.output_variables[self.pyomo_model.objective].__call__(self.model)
...
class PyomoMultiDataPointParamEstimator(ParameterEstimator):
    def __init__(self, pyomo_model, number_of_data_point):
        '''
    ...
    def build(self, problem_description):
        ...
        for op in self.problem_description.symbol_list['CV']:
