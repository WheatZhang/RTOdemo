...
class RTO_Plant_WO_reactor(PyomoModel):
    def __init__(self):
        self.output_variables = {
            ...
            'con': (lambda m: m.constraint_noise_free),
    ...
    def build_rto(self, wocstr, cv_func):
        ...
        def constraint_noise_free(m):
            return cv_func['XFr_G'].__call__(m) - 0.08
        wocstr.constraint_noise_free = Expression(rule=constraint_noise_free)

        def purity_constraint(m):
            return cv_func['XFr_G'].__call__(m)+m.XFr_noise['G'] - 0.08
