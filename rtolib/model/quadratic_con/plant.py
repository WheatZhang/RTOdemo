...
class quadratic_con_problem_plant(PyomoModel):
    def __init__(self):
    ...
    def build_body(self, model):
        ...
        def cost(m):
            return m.u1*m.u1+m.u2*m.u2+m.u1*m.u2 == m.cost_var
        model.cost = Constraint(rule=cost)
        def constr(m):
            return 1-m.u1+m.u2*m.u2+2*m.u2 == m.con_var
