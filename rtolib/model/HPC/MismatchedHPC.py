...
class RTO_Mismatched_HPC(PyomoModel):
    def __init__(self):
        ...
        self.initial_value_file = os.path.join(os.path.dirname(__file__) + r"\MismatchedOpt_InitValue.txt")
    ...
    def build_body(self, hpc_model):
        ...
        #
        # ======================================
        hpc_model.ColumnEffTray = Param(initialize=1)
        hpc_model.NitrogenOxygenKij = Param(initialize=-0.01238*1.05)
        hpc_model.ColumnUA = Param(initialize=1.8)
        # ======================================
        #
        #        Output noise
        #
        ...
        def MaCoolerSelfaV_Calc(m):
            return sum(
                [sum([m.MaCoolerSelfaij[c1, c2] * m.FeedMaMFrac[c1] * m.FeedMaMFrac[c2] for c1 in m.Component]) for c2
                 in
                 m.Component])
