from rtolib.core import RTO_Plant
from pyomo.environ import *

class RTO_Plant_HPC(RTO_Plant):
    def __init__(self):
        ...
        self.objective = lambda m: m.obj

    def build(self, test_model):
        ...
        # --------GNSplitter---------
        test_model.GNSplitterOutAMFlow = Var(initialize=730, bounds=(0, 2000))
        test_model.GNSplitterOutAMFlow.fixed = True
        test_model.CondensorPrdtMFlow = Var(initialize=98.77237353146775, bounds=(0, 200))
        ...
        #
        # ======================================
        test_model.ColumnEffTray = Param(initialize=0.7)
        test_model.NitrogenOxygenKij = Param(initialize=-0.01238)
        test_model.ColumnUA = Param(initialize=2)
        # ======================================
        #
        #        Manipulated Variables
        #
        # ======================================
        test_model.FeedTaMFlow = Var(initialize=320.000000, bounds=(0, 2000))
        test_model.FeedTaMFlow.fixed = True
        test_model.FeedMaMFlow = Var(initialize=1680.000000, bounds=(0, 3000))
        ...
        def _product_purity_con(m):
            return m.ColumnVapLvMFrac[60, 'Nitrogen'] >= 0.999

        test_model.product_purity_con = Constraint(rule=_product_purity_con)

        def _drain_con(m):
            return m.ReboilerLiqLvMFlow >= 10
        ...
        # ====================================
        def _obj(m):
            # TA cost + MA cost - GAN product
            return 4.07 * m.FeedTaMFlow + 1.71 * m.FeedMaMFlow

        test_model.obj = Objective(rule=_obj, sense=minimize)