...
def get_plant():
    ...
    parallel_wocstr.Fb_con = Constraint(rule=Fb_con)
    def profit(m):
        return sum([parallel_wocstr.reactor_block[i].profit for i in parallel_wocstr.block_index])
    parallel_wocstr.profit = Expression(rule=profit)
    return parallel_wocstr
...
if __name__ == "__main__":
    ...
    results = solver.solve(plant, tee=True)
    for i in plant.block_index:
        print("Fb[%d]=%.2f"%(i,value(plant.reactor_block[i].Fb)))
        print("Tr[%d]=%.2f"%(i,value(plant.reactor_block[i].Tr)))
    print("profit=%d"%value(plant.profit))
    from rtolib.util.init_value import to_template
    to_template(plant, "plant_init_value.txt")
