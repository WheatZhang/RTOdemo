...
from rtolib.core import PyomoSimulator, PyomoOptimizer, NoiseGenerator, PyomoModelWithModifiers,\
...
def generate_zero_noise_file():
    ...
    for i in range(max_iter):
        for j in range(len(noise_level)):
            for k in noise_level.keys():
                noise_generator.get_noise(i, j, k)
    noise_generator.save_noise("noise/noise0.txt")


def do_test():
    problem_description = copy.deepcopy(default_WOR_description)
    problem_description.symbol_list['CV']=('XFr_A','XFr_B','XFr_E','XFr_P','XFr_G')
    ...
    noise_generator.load_noise("noise/noise0.txt")

    solver_executable=r"../../external/bin/ipopt.exe"

    def post_iteration_callback(algo):
        print(algo.model_history_data)
        print(algo.plant_history_data)
        print(algo.input_history_data)
