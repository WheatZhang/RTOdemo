...
def draw_overall_pic_cn(folder, pic_filename):
    # pic_filename="pic/overall_pic0-0.png"
    # =======================================
    #            Load Data
    # =======================================
    ...
    plt.subplot(3, 1, 1)
    Optimal, = plt.plot(range(max_iter+1), ori_plant_cost, linewidth=global_linewidth, label='最优值', color='gray',
                        linestyle='--')
    ...
    plt.legend(loc='lower right',
               ...
               facecolor='w',
               framealpha=1.0,
               prop={'family': 'FangSong', 'size': global_tick_size})
    plt.ylabel('(1) 过程对象收益', font2)
    plt.xticks([0, 3, 6, 9, 12, 15, 18])
    plt.rcParams['xtick.direction'] = 'in'
    plt.rcParams['ytick.direction'] = 'in'
    plt.tick_params(labelsize=global_tick_size)
    ...
    plt.ylabel('(2) B进料流量, $F_B$ (kg/s)', font2)
    plt.xticks([0, 3, 6, 9, 12, 15, 18])
    plt.yticks([4, 4.5, 5, 5.5])
    plt.rcParams['xtick.direction'] = 'in'
    plt.rcParams['ytick.direction'] = 'in'
    ...
    # plt.legend(handles=[OptimalTR, MA, MAy, PA, GPE, ISOPE], prop=font1)
    plt.xlabel('RTO迭代步', font2)
    plt.ylabel('(3) 反应温度, $T_R$ (℃)', font2)
    plt.xticks([0, 3, 6, 9, 12, 15, 18])
    plt.rcParams['xtick.direction'] = 'in'
    plt.rcParams['ytick.direction'] = 'in'
    ax = plt.gca()
    ...
    plt.savefig(pic_filename, dpi=600)
    plt.close()
def draw_overall_pic_with_GPEAW(folder, pic_filename):
    # pic_filename="pic/overall_pic0-0.png"
    # =======================================
    #            Load Data
    # =======================================
    ...
    MA_plant_data = pandas.read_csv(folder + "MA_plant_data.txt", sep='\t', index_col=0, header=0)
    MA_input_data = pandas.read_csv(folder + "MA_input_data.txt", sep='\t', index_col=0, header=0)
    GPEAW_plant_data = pandas.read_csv(folder + "GPEAW_plant_data.txt", sep='\t', index_col=0, header=0)
    GPEAW_input_data = pandas.read_csv(folder + "GPEAW_input_data.txt", sep='\t', index_col=0, header=0)
    PE_plant_data = pandas.read_csv(folder + "PE_plant_data.txt", sep='\t', index_col=0, header=0)
    PE_input_data = pandas.read_csv(folder + "PE_input_data.txt", sep='\t', index_col=0, header=0)
    GPE_plant_data = pandas.read_csv(folder + "GPE_plant_data.txt", sep='\t', index_col=0, header=0)
    GPE_input_data = pandas.read_csv(folder + "GPE_input_data.txt", sep='\t', index_col=0, header=0)
    ...
    MA, = plt.plot(range(max_iter), MA_input_data.loc[range(max_iter),"Fb"], linewidth=global_linewidth, label='MA', linestyle='-',
                   marker='o', markersize=global_point_size)
    GPEAW, = plt.plot(range(max_iter), GPEAW_input_data.loc[range(max_iter),"Fb"], linewidth=global_linewidth, label='GPEAW', linestyle='-',
                    marker='o', markersize=global_point_size)
    PA, = plt.plot(range(max_iter), PE_input_data.loc[range(max_iter),"Fb"], linewidth=global_linewidth, label='PE',
                   linestyle='-', marker='o', markersize=global_point_size)
    ISOPE, = plt.plot(range(max_iter), ISOPE_input_data.loc[range(max_iter),"Fb"], linewidth=global_linewidth, label='PE+MA',
    ...
    MA, = plt.plot(range(max_iter), MA_input_data.loc[range(max_iter),"Tr"], linewidth=global_linewidth, label='MA', linestyle='-',
                   marker='o', markersize=global_point_size)
    GPEAW, = plt.plot(range(max_iter), GPEAW_input_data.loc[range(max_iter),"Tr"], linewidth=global_linewidth, label='GPEAW', linestyle='-',
                    marker='o', markersize=global_point_size)
    PA, = plt.plot(range(max_iter), PE_input_data.loc[range(max_iter),"Tr"], linewidth=global_linewidth, label='PE',
                   linestyle='-', marker='o', markersize=global_point_size)
    ISOPE, = plt.plot(range(max_iter), ISOPE_input_data.loc[range(max_iter),"Tr"], linewidth=global_linewidth, label='PE+MA',
    ...
    # plt.legend(handles=[OptimalTR, MA, GPEAW, PA, GPE, ISOPE], prop=font1)
    plt.xlabel('RTO Iterations', font2)
    plt.ylabel('(c) CSTR Temperature, $T_R$ (℃)', font2)
    plt.xticks([0, 3, 6, 9, 12, 15, 18])
    plt.rcParams['xtick.direction'] = 'in'
...
def draw_overall_pic_no_MAy(folder, pic_filename):
    # pic_filename="pic/overall_pic0-0.png"
    # =======================================
    #            Load Data
    # =======================================
...
def draw_GPES2_different_weight():
    pic_filename="pic/gpeS2-different-weight.png"
    # =======================================
    #            Load Data
    # =======================================
    weights=[0.1,1,10,100,1000]
    ...
    input_data = []
    for i, n in enumerate(weights):
        plant_data.append(pandas.read_csv("data/batch_GPES2_%s/GPES2_plant_data.txt"%str(n), \
                                               sep='\t', index_col=0, header=0))
        input_data.append(pandas.read_csv("data/batch_GPES2_%s/GPES2_input_data.txt" % str(n), \
                                               sep='\t', index_col=0, header=0))
...
def plot_contour(data_file,title,cn=False):
    global_font_size = 20
...
def plot_model_maintenance_cn(algo_results_folder, datafile_folder, pic_filename):
    from rtolib.core.solve import PyomoOptimizer
    from rtolib.model.wo_reactor import RTO_Plant_WO_reactor, RTO_Mismatched_WO_reactor, default_WOR_description
    from pyomo.environ import SolverFactory
    data_files={"Plant":datafile_folder+"_Plant.txt",
    ...
    plt.rcParams['xtick.direction'] = 'in'
    plt.rcParams['ytick.direction'] = 'in'
    plt.xlabel('B进料流量, $F_B$ (kg/s)', font2)
    plt.ylabel('反应温度, $T_R$ (℃)', font2)
    plt.tick_params(labelsize=global_tick_size)
    plot_contour(data_files['Plant'],  '(1) 过程对象', cn=True)
    ...
    plt.rcParams['xtick.direction'] = 'in'
    plt.rcParams['ytick.direction'] = 'in'
    plot_contour(data_files['Model'],  '(2) 对象模型', cn=True)
    plt.xlabel('B进料流量, $F_B$ (kg/s)', font2)
    plt.tick_params(labelsize=global_tick_size)
    ax = plt.gca()
    labels = ax.get_xticklabels() + ax.get_yticklabels()
    ...
    plt.rcParams['xtick.direction'] = 'in'
    plt.rcParams['ytick.direction'] = 'in'
    plot_contour(data_files['PE'],  '(3) PE')
    plt.xlabel('B进料流量, $F_B$ (kg/s)', font2)
    plt.tick_params(labelsize=global_tick_size)
    ax = plt.gca()
    labels = ax.get_xticklabels() + ax.get_yticklabels()
    ...
    plt.rcParams['xtick.direction'] = 'in'
    plt.rcParams['ytick.direction'] = 'in'
    plot_contour(data_files['ISOPE'],  '(4) PE+MA')
    plt.xlabel('B进料流量, $F_B$ (kg/s)', font2)
    plt.tick_params(labelsize=global_tick_size)
    ax = plt.gca()
    labels = ax.get_xticklabels() + ax.get_yticklabels()
    ...
    plt.rcParams['xtick.direction'] = 'in'
    plt.rcParams['ytick.direction'] = 'in'
    plot_contour(data_files['GPE'],  '(5) GPE')
    plt.xlabel('B进料流量, $F_B$ (kg/s)', font2)
    plt.tick_params(labelsize=global_tick_size)
    ax = plt.gca()
    labels = ax.get_xticklabels() + ax.get_yticklabels()
...
if __name__ == "__main__":
    ...
    # draw_overall_pic_no_MAy("data/1/", "pic/overall_pic_no_may.png")
    # draw_overall_pic_with_GPEAW("data/1/", "pic/overall_pic_with_GPEAW.png")
    # draw_overall_pic_cn("data/1/", "pic/overall_pic_cn.png")
    plot_model_maintenance_cn("data/1/","data/contour_data/res40",\
                              "pic/contour_pic_cn.png")