...
def draw_profile_trust_radius():
    ...
    global_marker_size = 2
    linewidth = 1
    fig = plt.figure(figsize=(12 * pic_constant, 15 * pic_constant))
    for ax_no in [211]:
        plt.subplot(ax_no)
        optimal = 1.453659e-01 * numpy.ones(max_iter + 1)
        plt.plot(range(max_iter + 1), optimal, linewidth=linewidth, label='Optimal', color='pink',
                 linestyle='--')
        plt.ylim([-1,5])
        plt.grid(ls='--',axis='y')
        plt.ylabel("cost", font_label)
    ...
    colors = ['darkblue', 'darkred', 'black', 'darkgreen']
    for no,prefix in enumerate(prefixes):
        ...
        plt.plot(range(1, max_iter + 1), plant_data.loc[1:max_iter, 'cost'], \
                 marker='o', c=colors[no], markersize=global_marker_size, \
                 linewidth=linewidth,label=labels[no])

        plt.subplot(212)
        plt.plot(range(1, max_iter + 1), plant_data.loc[1:max_iter, 'con'], \
                 marker='o', c=colors[no], markersize=global_marker_size, linewidth=linewidth,\
                 label=labels[no])
...
def plot_contour(data_file,title,contour_values):
    ...
    cmap = LinearSegmentedColormap.from_list('name', ['gold', 'blue'])
    # cmap = "jet_r"
...
def draw_contour_xi():
    ...
    for no, prefix in enumerate(prefixes):
        ...
        for iter in range(max_iter - 1):
            ...
            plt.plot(x, y, color=colors[no], linewidth=line_width)
        ...
        for iter in range(max_iter):
            if iter == 0:
                plt.plot(input_data.loc[iter, 'u1'], input_data.loc[iter, 'u2'], \
                         marker='o', c=colors[no], markersize=global_marker_size, label=labels[no])
            else:
                plt.plot(input_data.loc[iter, 'u1'], input_data.loc[iter, 'u2'], \
                         marker='o', c=colors[no], markersize=global_marker_size)
